...
$("#btn_add_doc").on('click', function() {
    ...
    if (!doc_name || !doc_link) {
        ...
        var check = confirm('Tài liệu thêm sẽ không thể sửa đổi. Bạn có chắc chắn muốn thêm không?');
        if (check == true) {
...
$(".btn_delete_doc").on('click', function() {
    var check = confirm('Bạn có muốn xóa tài liệu này không?');
    if (check == true) {
        var doc_id = $(this).val();
        $.ajax({
            type: "POST",
            url: './process_delete_doc.php',
            data: {
                doc_id: doc_id,
            },
            success: function(response) {
